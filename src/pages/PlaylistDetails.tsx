...
const PlaylistPage = () => {
  ...
  const contextPlaylist = getPlaylistById(playlistId);

  const [playlist, setPlaylist] = React.useState<any>(contextPlaylist);
  const [isPlaylistLoading, setIsPlaylistLoading] = React.useState(!contextPlaylist);
  ...
  // Both camelCase and snake_case check for safety
  const isPredefined = playlist?.isPredefined || playlist?.is_predefined || false;
  ...
  React.useEffect(() => {
    if (contextPlaylist) {
      ...
      apiClient.get("playlists.php", { action: "get", id: playlistId })
        .then((data) => {
          if (data && !data.error) {
            setPlaylist(data);
          }
  ...
  React.useEffect(() => {
    if (playlist) {
      ...
      setHasMore(true);
      setPodcasts([]);
      fetchPlaylistPodcasts(0);
    }
  }, [playlist?.id]);

  const fetchPlaylistPodcasts = async (offsetToFetch: number) => {
    if (!playlist) return;
  ...
  const handleLoadMore = () => {
    if (!isLoading && hasMore) {
      ...
      setOffset(newOffset);
      fetchPlaylistPodcasts(newOffset);
    }
  };
  ...
  const handleDeleteAll = async () => {
    ...
    try {
        ...
        toast({
            ...
            description: `All downloaded audios have been removed.`,
        });
    } catch(e) {
        toast({
            variant: 'destructive',
        ...
        <div className="flex h-screen flex-col bg-background">
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <SidebarInset className="flex flex-1 flex-col items-center justify-center">
              ...
              <p className="mt-4 text-muted-foreground">Loading playlist data...</p>
            </SidebarInset>
          </div>
          <BottomNavBar />
        </div>
              ...
              <main className="p-4 sm:p-6 lg:p-8">
                ...
                {podcastsInPlaylist.length > 0 ? (
                  <>
                    ...
                    {!hasMore && podcastsInPlaylist.length > 0 && (
                      <p className="mt-4 text-center text-sm text-muted-foreground pb-24">

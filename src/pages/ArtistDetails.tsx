...
const ArtistPage = () => {
  ...
  // Reset offset and fetch when artist changes
  React.useEffect(() => {
    setOffset(0);
    setHasMore(true);
    fetchArtistPodcasts(0);
  }, [artistName]);

  const fetchArtistPodcasts = async (offsetToFetch: number) => {
    setIsLoading(true);
    try {
  ...
  }, [artistName, allPlaylists, getPodcastsForPlaylist, podcasts]);
              ...
              <main className={cn("p-4 sm:p-6 lg:p-8", "pb-24 md:pb-8")}>
                ...
                <Tabs defaultValue="audios" className="w-full">
                  ...
                  <TabsContent value="audios">
                    ...
                    {!hasMore && podcastsByArtist.length > 0 && (
                      <p className="mt-8 text-center text-sm text-muted-foreground">
                        No more podcasts to load
                      </p>
                    )}
                    
                    {podcastsByArtist.length === 0 && !isLoading && (
                      <div className="flex h-48 items-center justify-center rounded-md border border-dashed">
                        <p className="text-muted-foreground">
                          No podcasts found for this artist.
                        </p>
                      </div>
                    )}
                  </TabsContent>
